import '../sass/buySucces.scss';
import { getUrlData } from '../static/util/getUrlData';
...
async function movieNameReader(moviesArr, cinemaArr) {
    ...
    const moviesObj = orderData.data.movieId;
    const operaObj = orderData.data.operaId;

    // console.log(moviesObj,operaObj)

    let movieName = `<h4>${moviesObj.title}</h4>`;
    let cinemaName = `<p>${operaObj.name} • 英文 2D </p>`;
    let movieImg =  `<img src="${moviesObj.imgSrc}" alt="">`;
    ...
    $('.image').html(movieImg);
    $('.text h5').text(`订单号：${orderData.data._id}`)
}
